...
import type { IUrsamuSDK, FormatSlot } from "@ursamu/ursamu";
...
export async function doBBList(u: IUrsamuSDK): Promise<void> {
  ...
  for (const [cat, catBoards] of cats) {
    ...
    for (const board of catBoards) {
      ...
      const rowOverride = await resolveGlobalFormat(u, "BBROWFORMAT" as FormatSlot, defaultRow);
  ...
  const blockOverride = await resolveGlobalFormat(u, "BBFORMAT" as FormatSlot, defaultBlock);
...
export async function doListPosts(u: IUrsamuSDK, boardStr: string): Promise<void> {
  ...
  for (const post of bPosts) {
    ...
    for (let i = 0; i < (post.replies ?? []).length; i++) {
      ...
      const replyOverride = await resolveGlobalFormat(u, "BBROWFORMAT" as FormatSlot, replyDefault);
