 ...
 *
 * The attack: a staff member sets a webhook URL pointing at an internal service
 * (localhost, private RFC-1918 range, link-local) to probe or exfiltrate data.
 *
 * Red tests: these must FAIL before the patch (the blocked URLs currently pass
 * the https:// prefix check and would be accepted).
 */
import { assertEquals } from "@std/assert";
import { describe, it } from "@std/testing/bdd";
import { isWebhookUrlSafe } from "../src/url-safety.ts";

// Inline the vulnerable "before" state to document the exploit
function isWebhookUrlSafeVulnerable(url: string): boolean {
  return url.startsWith("https://"); // original — no private-IP check
}

// The patched implementation is now the real one
const isWebhookUrlSafePatched = isWebhookUrlSafe;

// ---------------------------------------------------------------------------
// Red tests — demonstrate the vulnerability in the CURRENT (vulnerable) code
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Green tests — patched implementation must block all of the above
// ---------------------------------------------------------------------------
